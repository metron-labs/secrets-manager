...
import { BLOB_HEADER } from "./constants";
...
export async function encryptBuffer(azureKvStorage:  AzureKeyValueStorage,message: string): Promise<Buffer> {
        try {
            ...
            try {
                response = await azureKvStorage.cryptoClient.wrapKey('RSA-OAEP', key);
...
export async function decryptBuffer(azureKeyValueStorage: AzureKeyValueStorage,ciphertext: Buffer): Promise<string> {
        try {
            ...
            let key;
            try {
                const response = await azureKeyValueStorage.cryptoClient.unwrapKey('RSA-OAEP', encryptedKey);
...
export function isJson(content: Buffer): boolean {
        try {
